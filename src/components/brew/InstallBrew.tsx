import { BREW_INSTALL_COMMAND } from "../../constants/brew";
...
export function InstallBrew({ onRecheck, checking }: InstallBrewProps) {
  ...
  return (
    <div className="install-brew">
      ...
      <p>
        Brew Versionneer needs the Homebrew CLI to list installed packages. Install Homebrew using
        the command below, then click Check again.
      </p>

      <section>
        <h2>{isMac ? "macOS" : "Linux / macOS"}</h2>
        <pre className="install-command">{BREW_INSTALL_COMMAND}</pre>
        {!isMac && (
          <p className="install-note">
            See{" "}
            <a href="https://docs.brew.sh/Homebrew-on-Linux" target="_blank" rel="noreferrer">
              Homebrew on Linux
            </a>{" "}
